...
class RedundancyChecker:
    ...
    def add_entry(self, match, solution):
    ...
    def is_redundant(self, match):
        ...
        for e in self.lookup_table:
            if tuple(key) in permutations(iter(e), len(key)):
                print('Combination found in lookup table.')
                return True, e[3]
        return False, -1

def output_solutions(lines, blas_location, csv_file, is_short, enable_skip_check):
...
#    for s_bench, s_krnl in zip(lines_of_file[0::2], lines_of_file[1::2]):
        ...
        if mres == None:
            ...
            if enable_skip_check:
                skip_tuning, solution_index = checker.is_redundant(mres)
            ...
            print('Tuning command:', s_tuning)

            if not skip_tuning:
                solution_index, tuned_time = run_tuning(s_tuning, blas_location)
                #solution_index, _ = run_tuning(s_tuning, blas_location)
                if enable_skip_check:
                    checker.add_entry(mres, solution_index, tuned_time)

            print('Tuned performance: {}%'.format(((bench_time-tuned_time)/tuned_time)*100))
...
def main():
    ...
    if args.blas_tuning or args.blas_tuning_slow:
        ...
        with open(args.csv_file, 'a') as f_out:
            ...
            output_solutions(lines, args.blas_location, f_out, is_short, False)
